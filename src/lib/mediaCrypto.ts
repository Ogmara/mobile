...
// (was reported live on desktop: an image failed here, then decoded fine
// after switching channels and back — same CID, same key, just needed more
// time). Also feeds the disk-cache TTL check no differently — a cache miss
// still goes through this same retry loop.
